...
def evaluate_model(model, dataloader, device, logger, phase=3, is_sample_run=False):
    """
    ...
    with torch.no_grad():
        ...
        for batch_idx, (frames, labels) in enumerate(progress_bar):
            ...
            # labels: [inst, verb, target, triplet, video_ids]
            triplet_gt = labels[3].cpu().numpy()
            video_ids = labels[4]
            
            # Forward pass
            model.use_refiner = (phase == 3)
            logits_I, logits_V, logits_T, triplet_logits, _, _ = model(frames)
